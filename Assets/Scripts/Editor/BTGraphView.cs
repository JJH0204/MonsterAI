...
public class BTGraphView : GraphView
{
    ...
    public void RedrawTree()
    {
        ...
        if (!string.IsNullOrEmpty(selectedGuid) && _nodeViews.TryGetValue(selectedGuid, out var selectedView))
        {
            ...
            // 선택된 노드 위치로 뷰포트 이동
            FocusOnNode(selectedView.Node.position);
        }
    }
    ...
    public void FrameAllNodes()
    {
        ...
        // 3. 스케일 계산 (그래프 전체가 화면에 보이도록)
        float scaleX = viewWidth / bounds.width;
        float scaleY = viewHeight / bounds.height;
        float targetScale = Mathf.Min(scaleX, scaleY);
        // 바운드가 너무 작으면 1배로 고정
        if (bounds.width < 1f || bounds.height < 1f) targetScale = 1.0f;
        // 4. 위치 보정 (스케일 적용된 바운드 중앙이 뷰포트 중앙에 오도록)
        Vector3 targetPos = viewCenter - boundsCenter * targetScale;
        UpdateViewTransform(targetPos, new Vector3(targetScale, targetScale, 1));
    }
    ...
    public void FocusOnNode(Vector2 position)
    {
        var selectedView = GetSelectedNodeView();
        if (selectedView == null)
        {
            // 선택된 노드가 없으면 전체 노드가 화면에 보이도록 프레임
            FrameAllNodes();
            return;
        }
        // GraphView(윈도우)의 중앙 좌표 계산
        var viewCenter = this.layout.size * 0.5f;
        // 노드의 크기(가로/세로) 가져오기 (기본값: 200x100)
        float nodeWidth = selectedView.layout.width > 0 ? selectedView.layout.width : 200f;
        float nodeHeight = selectedView.layout.height > 0 ? selectedView.layout.height : 100f;
        // 노드의 중앙 좌표 계산
        Vector2 nodeCenter = position + new Vector2(nodeWidth * 0.5f, nodeHeight * 0.5f);
        // 뷰포트 위치 계산
    ...
    {
        if (_nodeViews.TryGetValue(guid, out var view))
            return view;
        return null;
    }
}
