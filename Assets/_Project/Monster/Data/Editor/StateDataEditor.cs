...
public class StateDataEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            var stateData = (StateData)target;
            // 기본 상태와 중복 체크
            ...
            if (!isDuplicate)
            {
                ...
                var newStates = stateData.states == null ?
                    new StateData.StateDefinition[1] :
                    new StateData.StateDefinition[stateData.states.Length + 1];
                if (stateData.states != null)
                    stateData.states.CopyTo(newStates, 0);
                newStates[newStates.Length - 1] = new StateData.StateDefinition
                {
