...
namespace Monster.AI.Command
{
    public class AttackCommand : AICommand
    {
        ...
        public override IEnumerator Execute(Blackboard.Blackboard blackboard, Action onComplete)
        {
            ...
            int skillID = _skillRowData.id;
            float coolTime = _skillRowData.cooldownTime;
        ...
        private void FireBullet(Blackboard.Blackboard blackboard)
        {
            ...
            // Agent가 바라보는 방향으로 발사
            var direction = blackboard.Agent.transform.forward;
            
            blackboard.AttackInfo.Fire(blackboard.Agent, blackboard.AttackInfo.firePoint.position, Vector3.zero, direction, _skillRowData.damage);
        }
    }
}